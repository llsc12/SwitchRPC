...
enum Base64 {
	...
	public static func EncodeString(_ bytes: [UInt8], alphabet: Alphabet = .Standard)
	-> String
	{
		let encoded = Encode(bytes, alphabet: alphabet)
		// Allocate a buffer of UTF-16 code units (or use [Character])
		var result = ""
		result.reserveCapacity(encoded.count)
		for b in encoded {
			// For Base64, all bytes are ASCII (0...127) and safe to convert
			result.append(b.asciiCharacter)
		}
		return result
	}
	
	/// Get the encoding table for the alphabet.
	private static func tableForAlphabet(_ alphabet: Alphabet) -> [UInt8] {
		switch alphabet {
		case .Standard:
	...
	public static func Encode(_ bytes: [UInt8], alphabet: Alphabet = .Standard) -> [UInt8]
	{
		var encoded: [UInt8] = []
		...
		while i + 3 <= count {
			...
			let three =
			((bytes[i + 1] & 0b00001111) << 2) | ((bytes[i + 2] & 0b11000000) >> 6)
